        ...
        <div class="card">
            <div class="card-body">
                ...
                <?php if (has_permission('create_users')): ?>
                    <a href="<?= base_url('users/create') ?>" class="btn btn-outline-primary btn--icon-text"><i class="zmdi zmdi-collection-plus zmdi-hc-fw"></i>Add User</a>
                <?php endif; ?>
                <br><br>
...
<script type="text/javascript">
    ...
    function loadPagination(key_search,sort_by,order_sort,rowperpage,pageno){
        $.ajax({
            url: '<?=base_url()?>users/list_users/'+key_search+'/'+sort_by+'/'+order_sort+'/'+rowperpage+'/'+pageno,
            type: 'get',
            dataType: 'json',
    ...
    function createTable(result,sno){
        ...
        if (result.length > 0)
        {
            ...
            for(index in result){
                ...
                var name_account = result[index].name_account;
                var role_name = result[index].role_name;
                sno+=1;
                ...
                tr += "<td>"+ sno +"</td>";
                tr += "<td>"+ username +"</td>";                
                tr += "<td>"+ name_account +"</td>";
                tr += "<td>"+ role_name +"</td>";
                tr += `<td>
                            <?php if (has_permission('edit_users')): ?>
                                <a href="<?= base_url('users/edit/${id}') ?>" class="btn btn-outline-warning btn--icon-text"><i class="zmdi zmdi-edit zmdi-hc-fw"></i>Edit</a>
                            <?php endif; ?>
                            <?php if (has_permission('delete_users')): ?>
                                <a href="#!" class="btn btn-outline-danger btn--icon-text" onclick="deleteConfirm('${base_url}users/delete/${id}')"><i class="zmdi zmdi-delete zmdi-hc-fw"></i>Delete</a>
                            <?php endif; ?>
                        </td>`
                tr += "</tr>";
