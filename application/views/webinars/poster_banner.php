            ...
            <div class="card-body">
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="<?= base_url('webinars/edit/'.$webinar_id) ?>">Edit Webinar</a></li>
                ...
                ?>
                <?= form_open('webinars/create_edit_poster/'.$webinar_id) ?>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="contacts__item">
                                    <a class="contacts__img">
                                        <img id="img_landscape" src="<?= set_value('poster_banner1') ?: getPosterValue($posters, 'wbr_1280x720') ?: base_url().'assets/img/defaultuploadimage.png' ?>" alt="">
                                    </a>
                                    <a class="btn btn-light callfunction" data-toggle="modal" data-target="#modal-imagelandscape">Upload</a>
                                    <input name="poster_banner1" id="poster_banner1" type="hidden" class="form-control" value="<?= set_value('poster_banner1') ?: getPosterValue($posters, 'wbr_1280x720') ?>" required>
                                    <input name="poster_id1" id="poster_id1" type="hidden" value="<?= set_value('poster_id1') ?: getPosterValue($posters, 'wbr_1280x720', 'poster_id') ?>">
                                    <span class="invalid-feedback" style="display:block;"><?= form_error('poster_banner1') ?></span>
                                    <input name="poster_banner2" id="poster_banner2" type="hidden" class="form-control" value="<?= set_value('poster_banner2') ?: getPosterValue($posters, 'wbr_410x230') ?>" required>
                                    <input name="poster_id2" id="poster_id2" type="hidden" value="<?= set_value('poster_id2') ?: getPosterValue($posters, 'wbr_410x230', 'poster_id') ?>">
                                    <span class="invalid-feedback" style="display:block;"><?= form_error('poster_banner2') ?></span>
                                    <input name="poster_banner3" id="poster_banner3" type="hidden" class="form-control" value="<?= set_value('poster_banner3') ?: getPosterValue($posters, 'wbr_235x132') ?>" required>
                                    <input name="poster_id3" id="poster_id3" type="hidden" value="<?= set_value('poster_id3') ?: getPosterValue($posters, 'wbr_235x132', 'poster_id') ?>">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.callfunction').click(function () {
            ...
            $.ajax({
                type: "GET",
                url: '<?php echo base_url() ?>webinars/compare_image_banner',
        ...
        $('#uploadimagelandscape').click(function () {
            $.ajax({
                url: '<?php echo base_url() ?>webinars/get_token',
                success: function(data) {
                    $('#changing-contentimage1')
                    .html('<iframe src="http://wp.dens.tv/uploaders?w=1280&h=720&pw=448&ph=252&token='+data+'&thumb=50&CH=webinar_v1&type=image" width="100%" height="400px" frameborder="0"></iframe>');
                }
            });
        });
    ...
    function getUrlLandscape() {
        ...
        var image = 'https://picture.dens.tv/wp/img/webinar_v1/1280x720/'+img.replace(/.*\//, '');
        var image2 = 'https://picture.dens.tv/wp/img/webinar_v1/1280x720/thumbnail/'+img.replace(/.*\//, '');
