<style type="text/css">
    ...
    .highlightSort .btn-link {color: white !important; font-weight: 600; cursor: pointer;}
</style>
    ...
    <div class="content__inner">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <?php if (has_permission('create_vshorts')): ?>
                    <a href="<?= base_url('vshorts/create') ?>" class="btn btn-outline-primary btn--icon-text"><i class="zmdi zmdi-collection-plus zmdi-hc-fw"></i>Add Vshorts</a>
                <?php endif; ?>
                                ...
                                <table class="table table-striped table-inverse table-bordered mb-0" id="mytable1" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Title</th>
                                            <!-- <th>Image</th> -->
                                            <th class="highlightSort">Highlight</th>
                                            <th>Create Date</th>
                                            <th>Action</th>
                                        </tr>
                                    </thead>
...
<!--Active Highlight Confirmation-->
<div class="modal fade" id="activeHighlightTVModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <div class="modal-body" align="center">Are you sure?</div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                <a id="btn-confirmhighlightactive" class="btn btn-success" href="#">Yes</a>
            </div>
        </div>
    </div>
</div>

<!--Inactive Highlight Confirmation-->
<div class="modal fade" id="inactiveHighlightTVModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <div class="modal-body" align="center">Are you sure?</div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                <a id="btn-confirmhighlightinactive" class="btn btn-danger" href="#">Yes</a>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function sort_active(key_search,visible,sort_by,order_sort,rowperpage,pageno) {
        if (order_sort == 'asc') {
            var order_sort = 'desc'
        } else {
            var order_sort = 'asc'
    ...
    function loadPagination(key_search,sort_by,order_sort,visible,rowperpage,pageno) {
        $.ajax({
            url: '<?=base_url()?>vshorts/list_vshort/'+key_search+'/'+sort_by+'/'+order_sort+'/'+visible+'/'+rowperpage+'/'+pageno,
            type: 'get',
            dataType: 'json',
            success: function(response) {
                ...
                if (response.pagination=="") {
                    ...
                    if (visible=='Y') {
                        $('.highlightSort').html('<input type="button" class="btn btn-link" value="Highlight" onClick="sort_active(\''+key_search+'\',\''+visible+'\',\'highlight\',\''+order_sort+'\',\''+rowperpage+'\','+pageno+')" />')
    ...
    function createTable(result,sno) {
        ...
        if (result.length > 0) {
            ...
            for (index in result) {
                ...
                var image = result[index].thumbnail_url;
                var htmlimage = '<img id="myImg" src="' + image + '" alt="" width="100" height="100">'
                var title = result[index].title;
                var highlight = result[index].highlight;
                ...
                var htmlhighlight = ""
                if (highlight == '0') {
                    var highlight_status = "<button type='button' class='btn btn-outline-danger' disabled>Inactive</button>"
                    var htmlhighlight = '<a onclick="activeHighlightConfirm(\''+base_url+"vshorts/activated_highlight_vshort/"+id+'\')" href="#!" class="dropdown-item"><i class="zmdi zmdi-pin zmdi-hc-fw"></i>Activate Highlight</a>'
                } else {
                    var highlight_status = "<button type='button' class='btn btn-outline-success' disabled>Active</button>"
                    var htmlhighlight = '<a onclick="inactiveHighlightConfirm(\''+base_url+"vshorts/inactivated_highlight_vshort/"+id+'\')" href="#!" class="dropdown-item"><i class="zmdi zmdi-pin-off zmdi-hc-fw"></i>Deactivate Highlight</a>'
                }
                var activeinactive = ""
                if (visible==="N") {
                    activeinactive = "<a onclick='activeConfirm(\""+base_url+'vshorts/activated/'+id+"\")' href='#!' class='dropdown-item'><i class='zmdi zmdi-plus-circle zmdi-hc-fw'></i>Activate Vshorts</a>";
                } else {
                    activeinactive = "<a onclick='inactiveConfirm(\""+base_url+'vshorts/inactivated/'+id+"\")' href='#!' class='dropdown-item'><i class='zmdi zmdi-minus-circle zmdi-hc-fw'></i>Deactivate Vshorts</a>";
                }
                ...
                tr += `<td>
                            <div class='dropdown'>
                                ...
                                <?php if (has_permission('edit_vshorts')): ?>
                                    ${htmlhighlight}
                                    <a href="<?= base_url('vshorts/edit/${id}') ?>" class="dropdown-item"><i class="zmdi zmdi-edit zmdi-hc-fw"></i>Edit</a>
                                <?php endif; ?>
                                <?php if (has_permission('delete_vshorts')): ?>
                                    ${activeinactive}
                                <?php endif; ?>
                                </div>
                            </div>
    ...
    function activeHighlightConfirm(url){
        $('#btn-confirmhighlightactive').attr('href', url);
        $('#activeHighlightTVModal').modal();
    }

    function inactiveHighlightConfirm(url){
        $('#btn-confirmhighlightinactive').attr('href', url);
        $('#inactiveHighlightTVModal').modal();
    }
</script>